                                            ...
                                            <tr>
                                                ...
                                                <td>
                                                    @if ($item->product_thambnail == '' || $item->product_thambnail == null)
                                                        <img loading="lazy" src="{{ asset('upload/no_image.jpg') }}"
                                                            style=" width:90px !important;" alt="No Image">
                                                    @else
                                                        <img loading="lazy"
                                                            src="{{ asset('storage/products/thambnail/' . $item->product_thambnail) }}"
                                                            style="width: 90px; height: 90px; !important;">
                                                        {{-- <img loading="lazy"
                                                            src="{{ asset('storage/products/thambnail' . $item->product_thambnail) }}"
                                                            style="width: 90px; height: 90px; !important;"> --}}
                                                    @endif
                                                ...
                                                <td>{{ $item['category'] == null ? '' : $item['category']['category_name_en'] }}
                                                </td>
                                                <td>{{ $item['author'] == null ? '' : $item['author']['author_name_en'] }}
                                                </td>
                                                ...
                                                <td class="text-center">
                                                    {{ $item->product_units_id ? $item->product_units->product_units_name_en : ' ' }}
                                                </td>

                                                @if ($item->product_qty > 0)
                                                    <td class="text-center">{{ $item->product_qty }} <br>
                                                        <p class="badge badge-pill badge-success">Stock-In</p>
                                                    </td>
                                                @elseif ($item->product_qty <= 0)
                                                    <td class="text-center">{{ $item->product_qty }} <br>
                                                        <p class="badge badge-pill badge-danger">Stock-Out</p>
                                                    </td>
                                                @endif
                                                ...
                                                <td width="3%">
                                                    @if (
                                                        $item->sid_sps_section == 'medicin' ||
                                                            $item->sid_sps_section == 'Medicin' ||
                                                            $item->sid_sps_section == 'medicine' ||
                                                            $item->sid_sps_section == 'Medicine' ||
                                                            $item->sid_sps_section == 'medicines' ||
                                                            $item->sid_sps_section == 'Medicines' ||
                                                            $item->sid_sps_section == 'pharmacy' ||
                                                            $item->sid_sps_section == 'pharmecy' ||
                                                            $item->sid_sps_section == 'Pharmecy' ||
                                                            $item->sid_sps_section == 'Pharmacy' ||
                                                            $item->sid_sps_section == 'pharmacys' ||
                                                            $item->sid_sps_section == 'Pharmacys')
                                                        @if ($item->expired_date >= Carbon\Carbon::now()->format('Y-m-d'))
                                                            <span
                                                                class="badge badge-pill badge-warning">{{ $item->expired_date }}</span>
                                                        @else
                                                ...
                                                <td width="30%" class="text-center">
                                                    ...
                                                    <a href="{{ route('details-view-product', $item->id) }}"
                                                    ...
                                                    <a href="{{ route('product.delete', $item->id) }}"
                                                        class="btn btn-danger btn-sm delete_data" title="Delete Data"><i
