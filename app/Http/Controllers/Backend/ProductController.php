...
class ProductController extends Controller
{
    ...
    public function StoreProduct(Request $request)
    {
        ...
        if ($request->hasFile('product_thambnail')) {
            ...
            $product_thambnail = $request->file('product_thambnail');
            $filename = time() . '.' . $product_thambnail->getClientOriginalExtension();
            $destinationPath = storage_path('app/public/products/thambnail');
            $img = $manager->read($request->file('product_thambnail'));
            $img->resize(500, 500, function ($constraint) {
                $constraint->aspectRatio();
            })->save($destinationPath . '/' . $filename);
            Storage::putFileAs('public/products/thambnail', $product_thambnail, $filename);
        }
        ...
        foreach ($images as $img) {
            ...
            // Storage::putFileAs('public/products/multi-image', $filename);
    ...
    public function MultiImageAdd(Request $request)
    {
        ...
        $filename = null;
        if ($request->hasFile('multi_img_add')) {
            $manager = new ImageManager(new Driver());
            $multi_img_add = $request->file('multi_img_add');
            $filename = time() . '.' . $multi_img_add->getClientOriginalExtension();
            $destinationPath = storage_path('app/public/products/multi-image');
            $img = $manager->read($request->file('multi_img_add'));
            $img->resize(500, 500, function ($constraint) {
                $constraint->aspectRatio();
            })->save($destinationPath . '/' . $filename);
            Storage::putFileAs('public/products/multi-image', $multi_img_add, $filename);
        }
    ...
    public function ThambnailImageUpdate(Request $request, $id)
    {
        ...
        if (isset($request->product_thambnail)) {
            ...
            //remove old image
            if ($product_thambnail_img->product_thambnail) {
                $old_filename = public_path("storage\brand\\" . $product_thambnail_img->product_thambnail);
                if (file_exists($old_filename) != false) {
                    unlink($old_filename);
                }
            }
